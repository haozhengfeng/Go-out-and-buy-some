...
(function(){
	if($.Datatype){
		$.extend($.Tipmsg.w,{
			...
			"idcard":"身份证号码不对！",
			"m":"请填写手机号码！",
			"tender":"请输入以字母或数字开头,中间可以带有“-”的字符！",
			"machinenum":"请输入5-20位字母数字组合！",
			"menucode":"请输入正确编号"
		});
		
		$.extend($.Datatype,{
			
			"menucode":function(gets,obj,curform,regxp){
				if (/^(\d{4}D){0,3}\d{4}$/.test(gets)) {
					return true
				}
				return false
			},
			
			"machinenum":function(gets,obj,curform,regxp){
				if (/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{5,20}$/.test(gets)) {
					return true
				}
				return false
			},
			...
			"tender":function(gets,obj,curform,regxp){
				if (/^[a-zA-Z0-9]+(-[a-zA-Z0-9]+)*$/.test(gets)) {
					return true
				}
				return false
			},
			...
			*/
			...
			// 2014-12-14 郑兴苗 新增 账户正则
			"account":function(gets,obj,curform,regxp){
				//5-15位以字母开头的字母或者数字组合
				if (/^[a-zA-Z]{1}([a-zA-Z0-9_]){4,15}$/.test(gets)) {
					return true
				}
				return false
			},
			
			// 2014-12-14 郑兴苗 新增 密码正则
			"password":function(gets,obj,curform,regxp){
				//只能5-15位的字母和数字或下划线中的相互的组合
				var regis = new RegExp("(?![a-z]+$|[0-9]+$|_+$)^[a-zA-Z0-9_]{5,15}$");
				if( regis.test(gets) ){
					return true;
				}
				return false;
			...
			// 2014-12-13 郑兴苗 修改 手机正则验证
			"m":function(gets,obj,curform,datatype){
			...
			*/
			...
			"moneyWithCustom":function(gets,obj,curform,datatype){
				var decimail=$.trim(obj.attr("decimail"));
				var integer=$.trim(obj.attr("integer"));
				decimail =parseInt(decimail);
				integer =parseInt(integer);
				var regex = "^[1-9]{1,"+integer+"}(\\.\\d{1,"+decimail+"})?$|^0(\\.\\d{1,"+decimail+"})?$|^[1-9][0-9]{0,"+(integer-1)+"}(\\.\\d{1,"+decimail+"})?$";
